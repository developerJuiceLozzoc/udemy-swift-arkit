...
class ApplyingForcesViewController: UIViewController, ARSCNViewDelegate {
    ...
    func registerGestures() {
        ...
        let doubleTapGuesture = UITapGestureRecognizer(target: self, action: #selector(doubleTapped))
        tapGesture.numberOfTapsRequired = 1
        doubleTapGuesture.numberOfTapsRequired = 2
        tapGesture.require(toFail: doubleTapGuesture)
        self.sceneView.addGestureRecognizer(tapGesture)
        self.sceneView.addGestureRecognizer(doubleTapGuesture)

    }
    @objc
    func doubleTapped( _ gesture: UIGestureRecognizer) {
        guard let sceneView = gesture.view as? ARSCNView else { return }
        let touchLocation = gesture.location(in: sceneView) // 2d dimensional ??
        // these coordinates have nothing to do with scene kit
        
        // is this gesture 2d coordinates touching anything on the screen
        let hitResults = sceneView.hitTest(touchLocation, options: [:])
        
        if let hitResult = hitResults.first {
            let node = hitResult.node
            let accelerationVector = SCNVector3(x: hitResult.worldCoordinates.z, y: hitResult.worldCoordinates.y, z: hitResult.worldCoordinates.z)
            node.physicsBody?.applyForce(accelerationVector, asImpulse: true)
        }
    }
